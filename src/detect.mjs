...
export function detectJsRequired(document, opts = {}) {
  ...
  // Common SPA mount points with no server-rendered content inside.
  const emptyMount = ["#root", "#app", "#__next", "[data-reactroot]"].some((sel) => {
  ...
  return {
    ...
    reason: jsRequired
      ? emptyMount
        ? "empty SPA mount + external scripts"
        : "near-empty body + external scripts"
