// Guard: prove the suite runs on turbo-crawl, NOT a real browser. The register
// redirect rewrites `@playwright/test` to the shim, so `chromium.launch()` yields
// the turbo-crawl stub (version "turbo-crawl"); a real Playwright build would
...
test("the engine is turbo-crawl — no Chromium is launched", async () => {
  const browser = await chromium.launch();
  expect(browser.version()).toBe("turbo-crawl");
