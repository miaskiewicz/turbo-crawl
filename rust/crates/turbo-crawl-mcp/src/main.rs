//! turbo-crawl MCP server binary — newline-delimited JSON-RPC 2.0 over stdio.
//! No Node: one native process reads requests from stdin, writes responses to
//! stdout. The logic lives in the library (`turbo_crawl_mcp`).

use tokio::io::{AsyncBufReadExt, AsyncWriteExt, BufReader};
use turbo_crawl_mcp::{handle, Session};
